...
const EEGUpload = () => {
  ...
  const handleSubmit = async () => {
    if (!file) {
      setMessage("Please upload an EEG file first.");
      return;
    }
    ...
    try {
      ...
      const response = await fetch("http://localhost:4000/api/jobs/EMO-EEG/uploadeeg", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div style={styles.container}>
      ...
      <button onClick={handleSubmit} style={styles.button}>Submit EEG File</button>
